...
if st.session_state["page"] == "Home":
    ...
    st.markdown("""
        ...
        .btn-building-info button {
            background-color: #0061C2 !important;
        }
        .btn-assembly-info button {
            background-color: #215721 !important;
        }
        .btn-retrofit-info button {
            background-color: #DB5E21 !important;
        }
        </style>
    """, unsafe_allow_html=True)

    # Check if all Building Info dropdowns are completed
    building_info_keys = [
        "building_type",
        "building_structure",
        "heating_system",
    ...
    building_info_complete = all(
        st.session_state.get(k, "").strip() not in ["", "Select..."]
        for k in building_info_keys
    )

    # Check if all Assembly Info dropdowns are completed
    assembly_info_keys = [
        "assembly_walls",
        "window_door_frame",
        "window_glazing",
    ...
    assembly_info_complete = all(
        st.session_state.get(k, "").strip() not in ["", "Select...", "Please select building structure first"]
        for k in assembly_info_keys
    )

    # Check if all Retrofit Info dropdowns are completed
    retrofit_info_keys = [
    ...
    retrofit_info_complete = all(
        st.session_state.get(k, "").strip() not in ["", "Select..."]
        for k in retrofit_info_keys
    )
    ...
    with col1:
        ...
        st.markdown("""
            <form action="" method="post">
                <button type="submit" name="navigate" value="Building Info"
                    class="custom-square-btn btn-building-info">Building Info</button>
            </form>
        """, unsafe_allow_html=True)

        
    if "navigate" in st.session_state:
        st.session_state["nav"] = st.session_state["navigate"]
    elif "navigate" in st.experimental_get_query_params():
        st.session_state["nav"] = st.experimental_get_query_params()["navigate"][0]
        st.write("testing")
        if building_info_complete:
            st.write("Building Info has been completed.")
        else:
    ...
    with col2:
        st.markdown('<div class="custom-square-btn btn-assembly-info">', unsafe_allow_html=True)
        if st.button("Assembly Info", key="home_assembly_info"):
            st.session_state["nav"] = "Assembly Info"
        st.markdown('</div>', unsafe_allow_html=True)
        st.write("testing")
        if assembly_info_complete:
            st.write("Assembly Info has been completed.")
        else:
            st.write("Blank has not been completed. Ensure to complete all fields so the project can be run through the calibrated energy model.")
    with col3:
        st.markdown('<div class="custom-square-btn btn-retrofit-info">', unsafe_allow_html=True)
        if st.button("Retrofit Info", key="home_retrofit_info"):
            st.session_state["nav"] = "Retrofit Info"
        st.write("testing")
        if retrofit_info_complete:
            st.write("Retrofit Info has been completed.")
        else:
            st.write("Blank has not been completed. Ensure to complete all fields so the project can be run through the calibrated energy model.")
