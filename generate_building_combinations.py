...
# Wall types depend on structure
wall_types = {
    "Concrete": [
...
glazing_cavity = ["1/4\"", "1/8\""]  # Only for double glazing
...
header = [
    ...
    "Retrofit window frame", "Retrofit window glazing", "Wall Exterior Insulation", "Roofs retrofit"
]
...
# Write to CSV
with open("building_combinations.csv", "w", newline='', encoding="utf-8") as csvfile:
    writer = csv.writer(csvfile)
    writer.writerow(header)
    writer.writerows(rows)

print(f"CSV generated with {len(rows)} combinations.")